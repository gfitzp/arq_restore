#!/bin/bash
#
# setup-cloudfront-cdn.sh — create the CloudFront CDN used by arq_restore's
# CDN download path to fetch Arq backup data from a private S3 bucket without
# per-GB S3 egress charges.
#
# What it creates (idempotent — safe to re-run; existing resources are reused):
#   1. An RSA key pair for CloudFront signed URLs (private key stays local)
#   2. A CloudFront public key + key group (trusted signer for the distribution)
#   3. An S3 Access Point on the backup bucket. Its dot-free alias is used as
#      the CloudFront origin domain instead of the bucket name: bucket names
#      that contain dots break S3's wildcard TLS certificate for direct
#      CloudFront origins, and the alias approach works for all bucket names.
#   4. A CloudFront Origin Access Control (sigv4, always sign)
#   5. Optionally, a WAF WebACL that blocks all IPs except ALLOWED_CIDRS
#   6. A CloudFront distribution: GET/HEAD only, HTTPS only, signed URLs
#      required, caching disabled, IPv6 off (so WAF/policy IP checks see IPv4)
#   7. Bucket policy delegation to the access point + access point policy
#      granting s3:GetObject to CloudFront for this distribution only
#
# Requirements: aws CLI v2, openssl, python3, curl.
# The AWS profile must have CloudFront, WAFv2, S3 and S3 Control permissions
# (a restricted backup-only profile does NOT — use an admin profile).
#
# Usage:
#   ADMIN_PROFILE=myadmin BUCKET=my-arq-backup-bucket ./setup-cloudfront-cdn.sh
#
# BUCKET is required on the first run; it is saved to KEY_DIR/cdn-config.env
# and reused (along with REGION and PREFIX) on later runs and by teardown.
#
# Optional overrides (env vars):
#   REGION          bucket region (default: us-east-1)
#   PREFIX          name prefix for all resources (default: arq-restore-cdn)
#   ALLOWED_CIDRS   comma-separated IPv4 CIDRs for WAF allowlist
#                   (default: auto = current public IP /32)
#   ENABLE_WAF      1 to create/attach the WAF WebACL (default: 1)
#   KEY_DIR         where keys and outputs are stored (default: ~/.arq_restore_cdn)
#   ASSUME_YES      1 to skip the bucket-policy confirmation prompt
#
# NOTE: enrolling in a CloudFront flat-rate pricing plan (e.g. Pro, $15/mo for
# 50 TB) is a separate step done in the CloudFront console under
# "Pricing plans" — this script cannot do it for you.
...
# Reuse settings saved by a previous run unless overridden in the environment.
CDN_BUCKET=""; CDN_REGION=""; CDN_PREFIX=""
[ -f "$CONFIG_FILE" ] && . "$CONFIG_FILE"

BUCKET=${BUCKET:-$CDN_BUCKET}
[ -n "$BUCKET" ] || { echo "error: set BUCKET to the S3 bucket containing the Arq backups" >&2; exit 1; }
...
json.dump({"CallerReference": name, "Name": name,
           ...
           "Comment": "arq_restore CDN signed URL key"}, sys.stdout)
PY
...
# -------------------------------------------------------------- 4. access point
say "S3 access point (its dot-free alias becomes the CloudFront origin domain)"
...
if [ "$OAC_ID" = "None" ] || [ -z "$OAC_ID" ]; then
    OAC_ID=$(awsx cloudfront create-origin-access-control \
        --origin-access-control-config "{\"Name\":\"$PREFIX-oac\",\"Description\":\"arq_restore CDN\",\"SigningProtocol\":\"sigv4\",\"SigningBehavior\":\"always\",\"OriginAccessControlOriginType\":\"s3\"}" \
...
if [ "$ENABLE_WAF" = "1" ]; then
    ...
    if [ "$IPSET_ARN" = "None" ] || [ -z "$IPSET_ARN" ]; then
        ...
        echo "reusing IP set (NOTE: not updating its addresses; edit in console if your IP changed)"
    fi
...
# ------------------------------------------------------------------- 8. policies
say "Bucket policy (delegate object access to this account's access points)"
...
if [ -n "$EXISTING_POLICY" ] && [ "$EXISTING_POLICY" != "None" ]; then
    ...
    echo "existing bucket policy backed up to $POLICY_BACKUP"
fi
NEW_POLICY=$(EXISTING_POLICY="$EXISTING_POLICY" python3 - "$BUCKET" "$ACCOUNT" <<'PY'
import json, os, sys
...
sid = "ArqRestoreCdnDelegateToAccessPoints"
...
if [ "$ASSUME_YES" != "1" ]; then
    ...
    [ "$reply" = "y" ] || [ "$reply" = "Y" ] || { echo "aborted before bucket policy change"; exit 1; }
fi
awsx s3api put-bucket-policy --bucket "$BUCKET" --policy "$NEW_POLICY"
echo "bucket policy applied"

say "Access point policy (CloudFront -> GetObject for this distribution only)"
AP_POLICY=$(cat <<EOF
{
  "Version": "2012-10-17",
  "Statement": [{
...
Next steps:
  1. Wait for the distribution to deploy (5-15 min):
       aws cloudfront wait distribution-deployed --id $DIST_ID --profile $ADMIN_PROFILE
  2. Enroll in a CloudFront flat-rate pricing plan (Pro: \$15/mo, 50 TB) in the
     CloudFront console under "Pricing plans" — without it you pay ~\$0.085/GB.
  3. Smoke test (expect 403 "Missing Key" — proves signed URLs are enforced):
       curl -si "https://$DIST_DOMAIN/anything" | head -5
  4. arq_restore's CDN download path reads $CONFIG_OUT
EOF
